...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='''Analyze trajectory estimate in a folder.''')
    parser.add_argument('result_dir', type=str, help="Folder containing the groundtruth and the estimate.")
    parser.add_argument('--plots_dir', type=str, help="Folder to output plots", default='')
    parser.add_argument('--mul_trials', type=int, help='number of trials, None for single run', default=None)
    parser.add_argument('--mul_plot_idx', type=int, nargs="*", help='index of  trials for plotting', default=[0])
    parser.add_argument('--est_types', nargs="*", type=str, default=['traj_est'])
    parser.add_argument('--recalculate_errors', help='Deletes cached errors', action='store_true')
    parser.add_argument('--png', help='Save plots as png instead of pdf', action='store_true')
    parser.add_argument('--plot_scale_traj', help='whether to plot scale colored trajectory (slow)', action='store_true')
    ...
    for est_type_i, plot_dir_i in zip(args.est_types, plots_dirs):
        ...
        print(Fore.MAGENTA + ">>> Plotting absolute error for one trajectory...")
        ...
        # pu.plot_aligned_top(ax, plot_traj.p_es_aligned, plot_traj.p_gt, plot_traj.align_num_frames)
        # plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
        # fig.tight_layout()
        # fig.savefig(plot_dir_i+'/trajectory_top' + '_' + plot_traj.align_str + FORMAT, bbox_inches="tight")
        ...
        # plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
        # fig.tight_layout()
        # fig.savefig(plot_dir_i+'/trajectory_side' + '_' + plot_traj.align_str + FORMAT, bbox_inches="tight")

        # fig = plt.figure(figsize=(6, 3))
        # ax = fig.add_subplot(111, xlabel='Distance [m]', ylabel='Position Drift [m]', xlim=[0, plot_traj.accum_distances[-1]])
        # pu.plot_error_n_dim(ax, plot_traj.accum_distances, plot_traj.abs_errors['abs_e_trans_vec'], plot_dir_i)
        # ax.legend()
        # fig.tight_layout()

        ## x y z  translation err.  vs  distance
        fig1, ax1 = plt.subplots(figsize=(6, 3))
        ax1.plot(plot_traj.accum_distances, plot_traj.abs_errors['abs_e_trans_vec'][:, 0]*-1, linewidth=1.0, color='r', label='x')
        ax1.plot(plot_traj.accum_distances, plot_traj.abs_errors['abs_e_trans_vec'][:, 1]*-1, linewidth=1.0, color='g', label='y')
        ax1.plot(plot_traj.accum_distances, plot_traj.abs_errors['abs_e_trans_vec'][:, 2]*-1, linewidth=1.0, color='b', label='z')
        ax1.set_xlabel('Distance [m]', fontsize=10)
        ax1.set_ylabel('translation err. [m]', fontsize=10)
        ...
        # pu.plot_error_n_dim(ax, plot_traj.accum_distances, plot_traj.abs_errors['abs_e_ypr']*180.0/np.pi, plot_dir_i, labels=['yaw', 'pitch', 'roll'])
        # ax.legend()
        # fig.tight_layout()
        ...
        # pu.plot_error_n_dim(ax, plot_traj.accum_distances, np.reshape(plot_traj.abs_errors['abs_e_scale_perc'], (-1, 1)), plot_dir_i, colors=['b'], labels=['scale'])
        # ax.legend()
        # fig.tight_layout()
        # fig.savefig(plot_dir_i+'/scale_error'+'_'+plot_traj.align_str+FORMAT, bbox_inches='tight')

        # if args.plot_scale_traj:
        #     fig = plt.figure(figsize=(6, 12))
        #     ax_top = fig.add_subplot(211, aspect='equal', xlabel='x [m]', ylabel='y [m]', title='Top')
        #     ax_top.grid(ls='--', color='0.7')
        #     ax_side = fig.add_subplot(212, aspect='equal', xlabel='x [m]', ylabel='z [m]', title='Side')
        #     ax_side.grid(ls='--', color='0.7')
        #     abs_scale_e = np.abs( np.reshape(plot_traj.abs_errors['abs_e_scale_perc'], (-1, 1)),)
        #     color_idx =\
        ...
        #     fig.tight_layout()
        #     fig.savefig(plot_dir_i+'/scale_error_traj' + '_' + plot_traj.align_str + FORMAT, bbox_inches="tight")
        ...
        #                    labels, colors)
        # fig.tight_layout()
        # fig.savefig(plot_dir_i+'/rel_translation_error' + suffix + FORMAT, bbox_inches="tight")
        # plt.close(fig)
        ...
        # print(Fore.GREEN + "#### Done processing error type {0} ####".format(est_type_i))
